        ...
        if (stat && stat.isFile()) {
            fs.readFile(self.filePath, function (err, buf) {
                if (self.moduleCache[self.filePath]) {
                    self.moduleCache[self.filePath].content = buf ? buf.toString() : ''
                    self.moduleCache[self.filePath].filePath = self.filePath
                    self.moduleCache[self.filePath].dirPath = path.dirname(self.filePath)
                } else {
                ...
                for (var j = 0; j < fileList.length; j++) {
                    ...
                    self.moduleCache[_file] = {
                        ...
                        module_id: self.rootUrl + _file.substring(self.dirName.length + 1).replace(/\\\\/g, '\\')
                    }
