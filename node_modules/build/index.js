...
exports.start = function *() {
    ...
    if (yield isFile) {
        //如果不是js文件，则直接返回
        if (/\.js$/.test(this.url.replace(/\?[\s\S]+$/, '')) === false) {
            return fs.createReadStream(this.filePath)
        }

        //开始读取入口文件
        //首先记录入口文件
        this.mainFile = this.filePath
        yield exports.read
    ...
    Object.keys(self.moduleCache).forEach(function (key) {
        ...
        item.content =
            item.content.replace(SLASH_RE, "").replace(REQUIRE_RE, function (m, m1, m2) {
                if (m2) {
                    return '\r\nrequire(' + m1 + self.rootProtocol + path.join(path.dirname(item.module_id).
                        substring(self.rootProtocol.length), m2 + '.js') + m1 + ')\r\n'
                } else {
                    return m
                }
            })
        if (item.filePath === self.mainFilePath) {
            ...
            self.moduleBuffer += '\r\n;define("' + item.module_id + '",function(require,exports,module){\r\n' + item.content + '});\r\n'
        }
    })
