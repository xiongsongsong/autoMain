/*
 * todo:support directory index.js
 **/
    ...
    Object.keys(self.moduleCache).forEach(function (key) {
        ...
        item.content =
            item.content.replace(SLASH_RE, "").replace(REQUIRE_RE, function (m, m1, m2) {
                if (m2) {
                    return m.replace(m2, addJsExtname(self.rootProtocol + path.join(path.dirname(item.module_id).
                        substring(self.rootProtocol.length), m2)))
                } else {
                    return m
                }
            })
        if (item.filePath === self.mainFilePath) {
...
function save(callback) {
    ...
    fs.writeFile(filePath, this.moduleBuffer, function () {
        ...
        var ls = spawn('uglifyjs', [filePath, '--output', filePath, '--reserved', 'require,exports,module'], {
        ...
        ls.on('close', function (code) {
            fs.readFile(filePath, function (err, buf) {
                callback(null, buf ? buf.toString() : 'error')
            })
        });
    })
...
exports.readDir = function *() {
    return yield readDir
}

function readDir(callback) {
    var self = this
    fs.readdir(self.query.ls, function (err, ls) {
        if (err) {
        ...
        ls.forEach(function (item) {
            var _path = path.join(self.query.ls, item)
            try {
                var file = fs.statSync(_path)
                if (file.isFile() || file.isDirectory()) {
                    if (file.isFile() && path.extname(_path) === '.js') {
                        fileList.push({
                            value: self.rootProtocol +
                                path.join(
                                    global.baseUrl.replace(/^http(?:s)?:\/\//gmi, '') +
                                        _path.replace(global.baseDir, '').replace(/\w:/g, '')
                                ),
                            isJS: true
                        })
                    }
